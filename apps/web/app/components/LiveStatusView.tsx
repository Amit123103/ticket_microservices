      ...
      <form onSubmit={handleSearch} className="mb-8">
        <div className="flex flex-col sm:flex-row gap-3 rounded-2xl border border-stone-200 bg-white p-3 shadow-lg shadow-stone-200/30">
          <div className="relative flex-1 flex items-center bg-stone-50 rounded-xl px-4 py-3 border border-stone-200 focus-within:border-orange-400 focus-within:shadow-sm focus-within:shadow-orange-500/10 transition-all">
            <Icons.live className="h-5 w-5 text-orange-600 mr-3" />
        ...
        <div className="mt-3 flex items-center gap-2 text-xs text-stone-400">
          ...
          {Object.keys(LIVE_TRAINS).map((k) => (<button key={k} type="button" onClick={() => { setTrainNo(k); setResult(LIVE_TRAINS[k]); setErrorMsg(''); }}
            className="rounded-lg border border-stone-200 bg-white px-3 py-1.5 font-mono text-orange-600 hover:bg-orange-50 hover:border-orange-300 transition-all font-semibold">{k}</button>))}
        </div>
      </form>
          ...
          <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-lg shadow-stone-200/30">
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-stone-100 pb-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="grid h-10 w-10 place-items-center rounded-full bg-orange-50 border border-orange-200">
                  <Icons.activity className="h-5 w-5 text-orange-600 animate-pulse-soft" />
                </div>
              </div>
            </div>
            <div className="my-5 rounded-2xl bg-orange-50/80 p-5 border border-orange-200">
              <span className="text-[10px] font-bold uppercase tracking-widest text-orange-700">Current Location</span>
              <div className="flex items-center gap-3 mt-2">
                <Icons.mapPin className="h-6 w-6 text-orange-600 animate-pulse-soft" />
                <div>
            ...
            <div className="space-y-4">
              {result.stations.map((st: LiveRouteStation, idx: number) => (
                <div key={idx} className="flex items-start gap-4">
                  ...
                  <div className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold border-2 ${st.isPassed ? 'bg-orange-600 text-white border-orange-600' : 'bg-white text-stone-500 border-stone-300'}`}>{idx + 1}</div>
                  <div className={`flex-1 flex flex-wrap items-center justify-between gap-2 rounded-xl p-3.5 border ${st.isPassed ? 'bg-orange-50 border-orange-200' : 'bg-stone-50 border-stone-200'}`}>
                    <div>
                      <span className="font-bold text-stone-800 text-sm">{st.stationName}</span>
                      <span className="ml-2 text-[10px] font-bold text-orange-600">{st.stationCode}</span>
                    </div>
